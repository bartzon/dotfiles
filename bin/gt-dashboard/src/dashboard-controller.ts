...
export interface DashboardState extends RendererState {
  prs: PR[];
}

export class DashboardController implements InputHandler {
  private processor: PRProcessor;
  private cache: CacheManager;
  private renderer: Renderer;
  private state: DashboardState;
  private refreshInterval: NodeJS.Timeout | null = null;
  private isExiting = false;
  private inputProcessor?: any;
  private nonInteractive: boolean;
  private renderDebounceTimer?: NodeJS.Timeout;
  private pendingRenderUpdate = false;

  constructor(renderer: Renderer, options?: { nonInteractive?: boolean }) {
  ...
  setInputProcessor(processor: any) {
  ...
  private async loadPRs() {
    ...
    if (hasExistingPRs) {
      mergedPRs = this.state.prs.map(existingPR => {
        const basicPR = basicPRs.find(bp => 
          bp.repository.fullName === existingPR.repository.fullName && 
      ...
      for (const basicPR of basicPRs) {
        const exists = mergedPRs.find(pr => 
          pr.repository.fullName === basicPR.repository.fullName && 
  ...
  async executeAction(key: string, selectedPR: PR) {
  ...
  onKeyPress(input: string, key: any) {
  }
