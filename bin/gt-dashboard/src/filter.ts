...
interface FilterToken {
  type: 'text' | 'repo' | 'status' | 'stack' | 'number' | 'checks' | 'review' | 'merge';
  value: string;
  negate: boolean;
}

function parseFilterTokens(query: string): FilterToken[] {
...
function matchesToken(pr: PR, token: FilterToken): boolean {
  let matches = false;
