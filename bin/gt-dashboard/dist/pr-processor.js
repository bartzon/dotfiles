...
export class PRProcessor {
    ...
    async getProcessedPRs(limit = 20) {
        const rawPRs = await this.fetcher.fetchRecentPRs(limit);
        const basicPRs = rawPRs
            .map((pr) => this.parseBasicPR(pr))
            .filter((pr) => pr.state === 'open');
        return basicPRs;
    }
    async enrichPRs(prs, preserveOrder = false) {
    ...
    async enrichPR(pr, skipCache = false) {
    ...
    async getSliceInfo(repo, prNumber) {
        if (repo !== 'shop/world') {
            return undefined;
        }
    ...
    detectStacks(prs, preserveOrder = false) {
        const g = new Graph({ directed: true });
        const branchToPR = new Map();
        const numberToPR = new Map();
        for (const pr of prs) {
            if (pr.branch) {
        ...
        const orphanedPRs = prs.filter(pr => pr.baseBranch?.startsWith('graphite-base/') &&
        ...
        for (const orphan of orphanedPRs) {
            ...
            for (const candidate of repoPRs) {
                if (candidate.number >= orphan.number)
                    continue;
        ...
        for (const pr of prs) {
            ...
            if (root) {
                ...
                while (true) {
                    ...
                    if (!preds || preds.length === 0 || !componentNodes.includes(preds[0]))
                        break;
                    depth++;
                    current = preds[0];
                }
